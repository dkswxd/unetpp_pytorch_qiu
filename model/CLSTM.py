...
class ConvLSTM(nn.Module):
    def __init__(self, input_channel, num_filter, kernel_size, stride=1, padding=1,deform=False):
        super().__init__()
        if deform:
            self._conv = deform_conv.DeformConv2dPack(in_channels=input_channel + num_filter,
            ...
            self._conv = nn.Conv2d(in_channels=input_channel + num_filter,
                                   ...
                                   stride=stride,
                                   padding=padding)
        self._bn = nn.BatchNorm2d(num_filter*4, track_running_stats=False)
        ...
        # self.Wci = nn.Parameter(torch.zeros(1, num_filter, 1, 1)).cuda()
        # self.Wcf = nn.Parameter(torch.zeros(1, num_filter, 1, 1)).cuda()
        # self.Wco = nn.Parameter(torch.zeros(1, num_filter, 1, 1)).cuda()
