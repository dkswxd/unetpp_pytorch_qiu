...
class FCLSTM_unet(nn.Module):
    def __init__(self, config):
        super(FCLSTM_unet, self).__init__()
        ...
        self.BiCLSTM_feature_root = config['FCLSTM_feature_root']
        if config['loss'] == 'BCE':
        ...
        for layer in range(self.layers):
            ...
            if layer == 0:
                self.down_sample_convs['down{}'.format(layer)] = nn.Sequential(
                    self.get_conv_block(self.BiCLSTM_feature_root, feature_number, 'down{}'.format(layer)))
            else:
